...
"""
        ...
        # Шаг 3: Главный редактор (Llama-3.1-8B) - Юридическая и политическая проверка (Закон о СМИ РК)
        # Если есть нарушения, возвращает "REJECT"
        chief_editor_prompt = f"""Сіз Қазақстандық туристік БАҚ-тың Бас редакторысыз. 
Мәтінді Қазақстан Республикасының "Бұқаралық ақпарат құралдары туралы" Заңына, этикалық нормаларға және саяси корректілікке сәйкестігін тексеріңіз.
Егер мәтінде заң бұзушылық, арандатушылық немесе этикаға жат нәрсе болса, тек қана "REJECT" сөзін жазыңыз.
Егер бәрі дұрыс болса, дайын финалдық мәтінді қайтарыңыз.
...
"""
        ...
        if "REJECT" in final_text.upper() and len(final_text) < 20:
            logger.warning("Главный редактор отклонил новость по юридическим/политическим причинам.")
            return None
